...
uint      RF_mtry;
uint      RF_htry;
double    RF_k_for_alpha;
uint      RF_maxBootstrapSize;
...
double   *RF_riskTime; // risk time
uint     *RF_stratifier; // stratifier
int       RF_maxStrat;
uint     *RF_varCategories; // variable categories
double   *RF_categoryWeights; // category weights
int       RF_nCategories;
uint      *RF_toFix;  // variables to fix for consideration at every split
int       RF_nToFix; // number of variables to consider at every split
...
/* function to carry out bootstrapping */
...
void processDefaultGrow() {
  ...
  if ( (!(RF_opt & OPT_BOOT_TYP1) &&  (RF_opt & OPT_BOOT_TYP2)) ||
       ( (RF_opt & OPT_BOOT_TYP1) && !(RF_opt & OPT_BOOT_TYP2))) {
    RF_maxBootstrapSize = RF_observationSize;
...
char restoreNodeMembership(uint  r,
  ...
  if (rootFlag || ((RF_opt & OPT_BOOT_TYP1) && !(RF_opt & OPT_BOOT_TYP2))) {
    if ( (!(RF_opt & OPT_BOOT_TYP1) && !(RF_opt & OPT_BOOT_TYP2)) ||
         ( (RF_opt & OPT_BOOT_TYP1) &&  (RF_opt & OPT_BOOT_TYP2)) ) {
      bootMembrIndx  = uivector(1, RF_bootstrapSize[treeID]);
      bootMembrSize = RF_bootstrapSize[treeID];
    }
    else {
  ...
  if (rootFlag || ((RF_opt & OPT_BOOT_TYP1) && !(RF_opt & OPT_BOOT_TYP2))) {
    if ( (!(RF_opt & OPT_BOOT_TYP1) && !(RF_opt & OPT_BOOT_TYP2)) ||
         ( (RF_opt & OPT_BOOT_TYP1) &&  (RF_opt & OPT_BOOT_TYP2)) ) {
      free_uivector(bootMembrIndx, 1, RF_bootstrapSize[treeID]);
    }
    else {
...
void rfsrc(char mode, int seedValue) {
  ...
  case RF_GROW:
    ...
    j = (RF_nodeSize - 1) << 1;
    if (RF_maxBootstrapSize > j) { 
      j = RF_maxBootstrapSize - j;
    }
    else {
      j = 1;
    }
    ...
    if (impuritySummary) {
      ...
      while (selectRandomCovariates(treeID,
        ...
        if (nonMissImpuritySummary) {
          ...
          double **userFeature = dmatrix(1, 2, 1, nonMissMembrSize);
                ...
                if ((secondNonMissMembrLeftSize[r] > 0) && (secondNonMissMembrRghtSize[r] > 0)) {
                  ...
                  for (k = 1; k <= nonMissMembrSize; k++) {
                    if (secondNonMissMembrFlag[r][k] == TRUE) {
                      ...
                      userFeature[1][m] = RF_stratifier[repMembrIndx[nonMissMembrIndx[indxx[k]]]];
                      userFeature[2][m] = RF_riskTime[repMembrIndx[nonMissMembrIndx[indxx[k]]]];
                    }
                  }
                  if ((RF_rType[r] == 'B') ||
                      (RF_rType[r] == 'I') ||
          ...
          free_dmatrix(userFeature, 1, 2, 1, nonMissMembrSize);
...
/* Note: the actual computation of splits occurs in here. */
char selectRandomCovariates(uint     treeID,
                            Node     *parent,
                            uint     *repMembrIndx,
                            uint      repMembrSize,
  ...
  /* 
      The main action loop: while the number of candidate covariates
      is less than the user-specified (or default) # of candidate
      covariates per node (mtry), and there remain possible, 
      unconsidered candidate covariates, try to find covars that work. 

      NOTE: 
      In trying to implement some analogue of 'structured' ML (or 
      whatever you want to call it), this is probably the place where
      editing will need to happen. How that looks will depend 
      directly on how scientific structure is baked into the learning
      process. If variables are assigned to categories, this could
      work by requiring the specification of additional keyword args
      to the R function sitting above this C code, e.g. using some 
      vector x assuming categorical values, passed with as.factor(x)
      to the initial rfsrc(...) call. This would involve several 
      modifications upstream (this list is probably not exhaustive 
      yet):

        - additional SEXP added to the signature of rfsrcGrow()
          [src/randomForestSRC.c]; DONE
        - additional global var RF_something declared at the top 
          of this file
          [src/randomForestSRC.c]; DONE
        - assignment of the SEXP arg that is passed to rfsrcGrow() 
          to the RF_something global variable 
          [src/randomForestSRC.c]; DONE
        - modification of the R function rfsrc() signature to 
          reflect the additional keyword argument + its type/shape
          [R/rfsrc.R]; DONE
        - modification of the call to rfsrc() to reflect added 
          argument (maximally upstream)
          [run_edited.R, but malleable, bc that's only for testing]
        - modification of the .Call() which initiates the R-to-C
          jump to reflect the added argument
          [R/rfsrc.R]
  */

  /**
  * MODIFICATIONS TO MAKE FOR STRUCTURING:
  * --------------------------------------
  * Make changes to the below code. 
  * - execute the below code SEPARATELY for each variable category,
  *   where the new analog for mTry is catweight * mTry.
  */
  int cat_index;
  for (cat_index = 1; cat_index <= RF_nCategories; cat_index++) {

      // how many to try, of this category? how many already tried? 
      int num_to_try = (int) ceil(RF_mtry * RF_categoryWeights[cat_index]);
      int num_tried = 0;
      double nonz_count = 0.0;

      // re-initialize CDF, adjusting RF_xWeight to reflect the focus on this kind of variable //
      for (i = 1; i <= *uniformSize; i++) {
        if (RF_varCategories[i] == cat_index) {
          RF_xWeight[i] = 1;
          nonz_count++;
        }
        else RF_xWeight[i] = 0;
      ...
      for (i = 1; i <= *uniformSize; i++)
        RF_xWeight[i] /= nonz_count;

      // carry out the re-initialization //
      

      while ( ((*candidateCovariateCount) < RF_mtry) &&
            (candidateCovariate != 0) && (xVarFound == FALSE) && (num_tried < num_to_try)) {
        (*candidateCovariateCount) ++;
        num_tried++;
        ...
        (*covariate) = candidateCovariate = sampleFromCDF(ran1B,
                                                          ...
                                                          *cdfSize,
                                                          ...
                                                          density,
                                                          *densitySize);
        //printf("Covariate index: %u\n", *covariate);
        //fflush(0);

        // actual covariate for splitting found? //
        if (candidateCovariate != 0) {
          updateCDF(treeID,
                    RF_xWeightType,
                    RF_xWeight,
          ...
          // 
          //    don't carry out the split if there end up being fewer than
          //   2 observations on one side (e.g. too unbalanced); this 
          //    slows things down.  
          //
          if ((*nonMissMembrSize) < 2) {
            xVarFound = FALSE;
          ...
          // 
          //    if the given covariate is considered suitable for a split,
          //    pass to the indexx() function; code for this is strongly
          //    obfuscated, so it remains unclear what exactly that does. //
          if (xVarFound) {
            indexx((*nonMissMembrSize),
                   nonMissSplit,
                   (*indxx));
  ...
  /* repeat the same, but for the variables that have been specified as 
     "to be tried at every split" */
  int fixed_index = 1;
  for (fixed_index = 1; fixed_index <= RF_nToFix; fixed_index++) {

      // re-initialize CDF, adjusting RF_xWeight as before s.t. the current variable
      // to try has weight 1 (and all others have weight 0)
      for (i = 1; i <= *uniformSize; i++) {
        if (i == RF_toFix[fixed_index]) {
          RF_xWeight[i] = 1.0;
        }
        else RF_xWeight[i] = 0.0;
      }
  ...
  /* return non-zero value if at least one x-variable found for splitting */
  return xVarFound;
}
...
void stackTNQualitativeOutputObjects(char     mode,
  ...
  ulong localSize;
  if (RF_optHigh & OPT_MEMB_OUTG) {
    localSize = (ulong) RF_ntree * RF_maxBootstrapSize;
    *pRF_RMBR_ID_ = (uint*) stackAndProtect(&RF_nativeIndex, NATIVE_TYPE_INTEGER, RF_RMBR_ID, localSize, 0, RF_sexpString[RF_RMBR_ID], &RF_RMBR_ID_ptr, 2, RF_ntree, RF_maxBootstrapSize);
    localSize = (ulong) RF_ntree * RF_observationSize;
...
void stackPreDefinedCommonArrays() {
  ...
  if (RF_maxBootstrapSize > RF_observationSize ) {
    maxSize = RF_maxBootstrapSize;
  }
...
void acquireTree(char mode, uint r, uint b) {
  ...
  RF_bootMembershipIndex[b] = uivector(1, RF_bootstrapSize[b]); // is b the treeid?
...
void unstackAuxiliary2(char mode, uint b) {
  free_uivector(RF_bootMembershipIndex[b], 1, RF_bootstrapSize[b]);
...
/* grow the tree specified by treeID */
...
/**
* Called from rfsrc.R; takes R arguments (of type SEXP), 
* puts them into the correct C-usable formats, and stores
* as the global variables defined at the top of this file.
* Calls rfsrc() function with GROW parameter, which uses
* the global variables to set up the forest and grow the 
* trees.
*/
SEXP rfsrcGrow(SEXP traceFlag,  /* essentially a -verbose option */
               SEXP seedPtr,    
               SEXP optLow,     
               SEXP optHigh,
               SEXP splitRule,  /* rule for computing split statistics */
               SEXP nsplit,
               SEXP mtry,       /* number of covariates to try for split at each node */
               SEXP htry,
               SEXP ytry,
               SEXP nodeSize,   /* minimum node size */
               SEXP nodeDepth,
               SEXP crWeightSize,
               SEXP crWeight,
               SEXP ntree,            /* number of trees */
               SEXP observationSize,  /* number of observations */
               SEXP ySize,
               SEXP rType,
               SEXP rLevels,
               SEXP rData,
               ...
               SEXP riskTime,         /* risk time variable (for Poisson reg. split) */
               SEXP stratifier,       /* stratifying variable (e.g. interval) */
               SEXP maxStrat,         /* max stratifier value */
               SEXP varCategories,    /* category assignments for structured covariate sampling */
               SEXP categoryWeights,  /* weights for each of these categories */
               SEXP nCategories,
               SEXP toFix,
               SEXP nToFix,

               SEXP k_for_alpha,
               SEXP maxBootstrapSize,
               SEXP bootstrapSize,
               SEXP bootstrap,
               SEXP caseWeight,
  ...
  RF_k_for_alpha          = REAL(k_for_alpha)[0];
  /* modified to reflect change in bootstrap size structuring */
  RF_maxBootstrapSize     = INTEGER(maxBootstrapSize)[0];
  RF_bootstrapSize        = (uint *) INTEGER(bootstrapSize); RF_bootstrapSize--;
  ...
  RF_riskTime             = REAL(riskTime); RF_riskTime--;
  RF_stratifier           = (uint *) INTEGER(stratifier); RF_stratifier--;
  RF_maxStrat             = INTEGER(maxStrat)[0];
  RF_varCategories        = (uint *) INTEGER(varCategories); RF_varCategories--;
  RF_categoryWeights      = REAL(categoryWeights); RF_categoryWeights--;
  RF_nCategories          = INTEGER(nCategories)[0];
  RF_toFix                = (uint *) INTEGER(toFix); RF_toFix--;
  RF_nToFix               = INTEGER(nToFix)[0];
...
SEXP rfsrcPredict(SEXP traceFlag,
  ...
  RF_perfBlock            = INTEGER(perfBlock)[0];
  RF_numThreads           = 2;//INTEGER(numThreads)[0];
