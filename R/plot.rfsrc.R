...
#' 
#' Plot out-of-bag (OOB) error rates and variable importance (VIMP) from a
#' RF-SRC analysis. This is the default plot method for the package.
#' 
#' Plot cumulative OOB error rates as a function of number of trees and
#' variable importance (VIMP) if available.  Note that the default settings are
#' now such that the error rate is no longer calculated on every tree and VIMP
#' is only calculated if requested.  To get OOB error rates for ever tree, use
#' the option \code{tree.err = TRUE} when growing the forest or restore the
#' model using the option \code{tree.err = TRUE}.  Likewise, to view VIMP, use
#' the option \code{importance} when growing the forest or restore the forest
#' using the option \code{importance}.
#' 
...
#' @param verbose Should VIMP be printed?
#' @param ... Further arguments passed to or from other methods.
#' @author Hemant Ishwaran and Udaya B. Kogalur
#' @seealso \command{\link{predict.rfsrc}}, \command{\link{rfsrc}}
